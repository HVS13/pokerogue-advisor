...
declare module "#app/global-scene" { import type { PlayerIndex } from "#app/battle-scene"; import type { PokeballType } from "#enums/pokeball"; import type { EnemyPokemon, PlayerPokemon } from "#field/pokemon"; export const globalScene: { ui: { getMode(): number }; phaseManager: { getCurrentPhase(): { is(name: string): boolean; getFieldIndex?: () => number } }; currentBattle?: { turn: number }; twoPlayerMode: boolean; twoPlayerPartySize: 3 | 6; getPlayerIndexForFieldSlot(index: number): PlayerIndex; getPlayerPokeballCounts(index: PlayerIndex): Partial<Record<PokeballType, number>>; getPlayerParty(index: PlayerIndex): PlayerPokemon[]; getPlayerField(): PlayerPokemon[]; getEnemyField(): EnemyPokemon[]; }; }
